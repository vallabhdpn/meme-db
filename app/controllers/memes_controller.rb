class MemesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @meme.save
        format.html { redirect_to meme_url(@meme), notice: "Meme was successfully created." }
  ...
  def update
    respond_to do |format|
      if @meme.update(meme_params)
        format.html { redirect_to meme_url(@meme), notice: "Meme was successfully updated." }
  ...
  private
    ...
    def meme_params
      params.require(:meme).permit(:SlNo, :Title, :tags, :description, :folder)
    end
end
